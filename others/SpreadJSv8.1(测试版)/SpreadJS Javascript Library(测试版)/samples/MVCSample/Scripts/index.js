    ...
    $.getJSON(url, function (event) {
        if (event && event.data) {
            ...
            var columnInfos = [
                    ...
                    { name: "ModifiedDate", formatter: "mm/dd/yyyy", size: 120 },
            ];
            ...
            sheet.getColumn(0).hAlign(GcSpread.Sheets.HorizontalAlign.right);
            sheet.getColumn(2).hAlign(GcSpread.Sheets.HorizontalAlign.left);
...
function createData(sheet, url, callback) {
    ...
    sheet.setValue(rowCount, 11, new Date());
    sheet.isPaintSuspended(false);
